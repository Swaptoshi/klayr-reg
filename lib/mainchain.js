const {
  ...
  mainchainRegParams: mainchainRegParamsSchema,
  MESSAGE_TAG_CHAIN_REG,
} = require("klayr-sdk");
const { createMainchainClient, createSidechainClient } = require("./client");
...
const registerMainchainHanlder = async (logger, options) => {
  ...
  logger.verbose(
    `Sent mainchain registration transaction. Result from transaction pool is: ${result}`
  );
  ...
  if (options.authorizeCc) {
    try {
      ...
      logger.verbose(
        `Authorize Mainchain completed, result: ${authorizeMainchainResult}`
      );
    } catch (err) {
      logger.warn(
...
const registerMainchain = async (logger, options) => {
  try {
...
module.exports = { registerMainchain };
